...
scVioBox <- function(inpConf, inpMeta, inp1, inp2, 
                     inpsub1, inpsub2, inpH5, inpGene, 
                     inptyp, inppts, inpsplit, inpsiz, inpfsz){ 
  if(is.null(inpsub1)){inpsub1 = inpConf$UI[1]} 
  # Prepare ggData 
  ggData = inpMeta[, c(inpConf[UI == inp1]$ID, inpConf[UI == inpsub1]$ID), 
                   with = FALSE] 
  ggData$rowIdx = seq_len(nrow(inpMeta))
  colnames(ggData)[1:2] = c("X", "sub") 
  ...
  # Add split column using rowIdx (before factoring, so indices still valid)
  ggData$split = inpMeta$ordered.time.point[ggData$rowIdx]
  ...
  # Actual ggplot 
  if(inpsplit){
    timeCol = strsplit(inpConf[ID == "ordered.time.point"]$fCL, "\\|")[[1]]
    names(timeCol) = levels(inpMeta$ordered.time.point)
    if(inptyp == "violin"){
      ggOut = ggplot(ggData, aes(X, val, fill = split)) +
 ...
 optCrt="{ option_create: function(data,escape) {return('<div class=\"create\"><strong>' + '</strong></div>');} }" 
  ...
  output$sc1c1oup <- renderPlot({ 
    scVioBox(sc1conf, sc1meta, input$sc1c1inp1, input$sc1c1inp2,
             ...
             "sc1gexpr.h5", sc1gene, input$sc1c1typ, input$sc1c1pts, input$sc1c1split,
...
output$sc1c2oup.png <- downloadHandler( 
  ...
  output$sc2c1oup <- renderPlot({ 
    scVioBox(sc2conf, sc2meta, input$sc2c1inp1, input$sc2c1inp2,
         ...
         "sc2gexpr.h5", sc2gene, input$sc2c1typ, input$sc2c1pts, input$sc2c1split,
...
output$sc2c2oup.png <- downloadHandler( 
  ...
  output$sc3c1oup <- renderPlot({ 
    scVioBox(sc3conf, sc3meta, input$sc3c1inp1, input$sc3c1inp2, 
             ...
             "sc3gexpr.h5", sc3gene, input$sc3c1typ, input$sc3c1pts, input$sc3c1split,
...
output$sc3c2oup.png <- downloadHandler( 
  ...
  output$sc4c1oup <- renderPlot({ 
    scVioBox(sc4conf, sc4meta, input$sc4c1inp1, input$sc4c1inp2, 
             ...
             "sc4gexpr.h5", sc4gene, input$sc4c1typ, input$sc4c1pts, input$sc4c1split,
